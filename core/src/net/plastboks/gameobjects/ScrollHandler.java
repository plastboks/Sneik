...
public class ScrollHandler {
    ...
    public boolean collides(Snake snake) {
        if (!pipe1.isScored()
         && pipe1.getX() + (pipe1.getWidth() / 2) < snake.getX() + snake.getWidth())
        {
            addScore(1);
            pipe1.setScored(true);
            AssetLoader.coin.play();
        } else if (!pipe2.isScored()
                && pipe2.getX() + (pipe2.getWidth() / 2) < snake.getX() + snake.getWidth())
        {
            addScore(1);
            pipe2.setScored(true);
            AssetLoader.coin.play();
        } else if (!pipe3.isScored()
                && pipe3.getX() + (pipe3.getWidth() / 2) < snake.getX() + snake.getWidth())
        {
            addScore(1);
        ...
        return (pipe1.collides(snake)
             || pipe2.collides(snake)
             || pipe3.collides(snake));
    }
